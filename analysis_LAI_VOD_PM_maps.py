...
data=store['RWC_min_anomaly_grid']
store.close()
...
for year in year_range:   
    ...
    plot_data=m.scatter(lats, lons,s=marker_size,c=data_plot,cmap='RdPu_r',marker='s',\
                       vmin=-2.0,vmax=1)
cb1=fig.colorbar(plot_mort,ax=axs[0,:].ravel().tolist(), fraction=0.0054,\
                 aspect=20,pad=0.02)
cb2=fig.colorbar(plot_data,ax=axs[1,:].ravel().tolist(), fraction=0.0054,\
                 aspect=20,pad=0.02)
cb2.ax.invert_yaxis()
fig.text(0.1,0.67,'Fractional area \n Mortality',horizontalalignment='center',\
         verticalalignment='center',rotation=90)
fig.text(0.1,0.33,r'Min. $\frac{VOD_{pm}}{LAI}$'+'\n anomaly',\
...
#    data_plot=data_plot[data_plot>=-100]
    ...
    ax.set_ylabel('Kernel Density \n of  '+r'$\frac{VOD_{pm}}{LAI}$')
    ax.set_xlim([-3,2])
    ax.invert_xaxis()
plt.show()

#np.nanmin(data.iloc[:,:].values)
