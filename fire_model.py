...
#    log_model = LogisticRegression(penalty='l2',C=1)
    log_model = LogisticRegressionCV(penalty='l2',Cs=10, cv=10)
...
#cmap = cmap
#norm = mpl.colors.Normalize(vmin=0, vmax=27600/67500)
#
...
#    plt.annotate('f1={0:0.1f}'.format(f_score), xy=(0.6, y[45] + 0.02))
#
#lines.append(l)
...
#plt.legend(lines, labels, loc=(0, -.75), prop=dict(size=24))
#
#
#plt.show()
#--------------------------------------------------------------------------------

coef=pd.DataFrame(log_model.coef_,index=labels,columns=data_types)
coef=coef.abs()
