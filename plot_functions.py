...
def plot_timeseries(data_source1='mortality_%03d_grid',data_source2='RWC',data_source3='cwd',\
                            data_label1='Fractional area\nof mortality',\
                            data_label2='Relative water\ncontent',\
                            data_label3='Climatic water\n deficit (mm)',\
                            start_year=2009,end_year = 2015,grid_size=25,\
...
#    #           data[~mask]=np.nan
    publishable.set_figsize(0.5, 1, aspect_ratio =1)
...
#    ax.axhline(l,ls='dotted',color=color,lw=1)
    ...
    ax.scatter(idxquantile(data,q=0.95),data.quantile(0.95),s=30,c='None',lw=1,edgecolor=color,marker='s')
...
def plot_scatter(data1='RWC',data2='cwd',data1_label="Relative water content",\
                 data2_label='Climatic water deficit',
                    mort_label='Fractional area of mortality',\
...
#    ax.set_ylabel(mort_label)
    ...
    cbaxes = fig.add_axes([0.3, 0.55, 0.02, 0.15])
    cb=fig.colorbar(plot2_data,ax=axs[1],\
                    ticks=[min(z)+0.1*max(z), 0.9*max(z)],cax=cbaxes)
    cb.ax.set_yticklabels(['Low', 'High'])
    cb.ax.tick_params(axis='y', right='off',pad=-4)
    ...
    publishable.panel_labels(fig = fig, position = 'outside', case = 'lower',
                                                 prefix = '', suffix = '.', fontweight = 'bold')
    cb.outline.set_visible(False)
def plot_grid(data='data',journal='GlobEnvChange'):
    publishable = plotsettings.Set(journal)
    Df=pd.read_csv('D:/Krishna/Project/data/rf_%s.csv'%data,index_col=0)   
    input_sources=['live_basal_area','LAI_sum',\
    'RWC', 'aspect_mean', 'aspect_std','canopy_height',\
     'cwd','elevation_mean',\
     'forest_cover','ppt_sum','tmax_sum',\
     'tmean_sum','vpdmax_sum','EVP_sum',\
    'PEVAP_sum','vsm_sum']
